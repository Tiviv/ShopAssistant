...
| Phase 8 — Replace Supabase entirely | ✅ done | Every phase above kept Supabase as the default, with the Python backend as an optional secondary connection. This phase flips that: the Python backend becomes the *only* backend `index.html` talks to on this branch. New backend work: `GET/POST/PATCH /settings` (`backend/app/routers/settings.py`, the one resource that had never moved — company info, the three document counters, custom categories), and `POST /auth/forgot-password` + `POST /auth/reset-password` (`backend/app/routers/auth.py`) — single-use, SHA-256-hashed, 30-minute-expiry reset tokens (`PasswordResetToken` model), emailed via `aiosmtplib` (`backend/app/email.py`, any SMTP provider, `SMTP_HOST` empty just logs the link instead — fine for local dev). Frontend: `supabase-js` script tag, `supabaseClient`, `CONFIG_KEY`/project-URL-and-anon-key setup, and every `usingPyBackendFor()` branch removed from `index.html` — the `db` object's functions now unconditionally call the Python backend, and the old "Python backend (experimental)" Settings card is gone since there's no second connection to manage anymore. Access tokens bumped from a short expiry to 14 days (no refresh-token flow exists, and this is now the only login — see the comment in `app/config.py`). Verified end-to-end through the real rendered UI (not stubbed state) in headless Chromium: connect to backend → sign up → auto-login → product CRUD → log out → log back in (data persisted) → forgot password → real email caught by a local fake SMTP server → intact reset link (a link-corrupting line-wrap bug in the default `email.policy` was caught and fixed here) → reset via the token → old password rejected → new password works, zero page errors throughout. Separately verified customers/products/documents/cash-register CRUD and two-page realtime sync all still work through this same real-login path. |
...
- 2026-09-20 — Phase 8 done: Supabase removed entirely from this branch's
  `index.html`, replacing it as the app's sole backend rather than an
  optional secondary connection. This closes the two gaps every earlier
  phase had flagged and left open — the stale document-counter display and
  the two-connections-to-manage UX — by removing the reason for them
  (there's only one connection now). Two things had to be built that
  Supabase had provided and nothing here replaced yet: a `settings`
  endpoint (company info, counters, categories — the one resource that had
  stayed on Supabase this whole time) and password reset, since Supabase
  Auth's emailed-link flow had no equivalent. Password reset is now real
  SMTP (`aiosmtplib`, any provider, configured in `backend/.env` — empty
  `SMTP_HOST` just logs the link for local dev) with single-use, hashed,
  expiring tokens. One bug worth remembering: `EmailMessage`'s default
  policy soft-wraps body text at 78 characters, which silently split the
  reset link mid-token — fixed with a cloned policy at `max_line_length=998`.
  `index.html` lost the `supabase-js` tag, `supabaseClient`, the project-
  URL/anon-key setup screen, and every `usingPyBackendFor()` branch — the
  `db.*` seam still exists structurally but no longer branches on anything.
  Access tokens went from a short-lived expiry to 14 days, since there's no
  refresh-token flow and this is now the only way to stay logged in. `main`
  is untouched. Verified through the real UI end to end (connect → signup →
  auto-login → CRUD → logout/login → forgot-password → SMTP-caught email →
  reset → old password rejected, new one works) plus a second pass
  confirming customers/products/documents/cash-register CRUD and two-page
  realtime sync all still work through this real-login path, zero page
  errors. `docs/python-backend-plan.md`, `backend/README.md` and the root
  `README.md` updated to describe this branch's actual setup (a Postgres +
  FastAPI backend, no Supabase project needed) instead of the old
  optional-secondary-connection description.
